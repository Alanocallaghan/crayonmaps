...
setClass("crayonmap", 
  representation(crayons = "character", row.num = "numeric", col.num = "numeric")
)
...
#' @param na_colour Colour to be used for NA values
...
colorKey <- function(
  ...
  colours <- pal[
    as.numeric(cut(as.vector(values), breaks = breaks, include.lowest = TRUE))
  ]
  ...
  cols <- matrix(
    ...
    dimnames = list(rownames(values), colnames(values)))
...
#' @param dist_method Character, distance method passed to dist
#' @param clustering_method Character, clustering method passed to hclust
#' @param main Character, title of the heatmap
#' @param key_title Character, title of the color legend
...
textHeatmap <- function(
  ...
  crayonstrings <- matrix(
    unlist(
      sapply(
        colorKey(as.vector(dat), pal = pal),
        function(x) {
...
textDendrogram <- function(hc, horiz = FALSE, yend = 20, lpad = "") {
  ...
  segs[, c("y", "yend")] <- lapply(segs[, c("y", "yend")],
    scales::rescale,
    to = if (horiz) yscale else rev(yscale)
  )
  if (!horiz) {
...
#' Text-based heatmap
#' @param clust hierarchical clustering object return by hclust()
...
textDendro <- function(
  ...
  names(all_x) <- c(
    paste0("hor_xst", 1:nrow(segs_hor_red)),
  ...
  names(all_y) <- c(
    paste("hor_yst", 1:nrow(segs_hor_red)),
  ...
  for (i in seq_len(nrow(segs_all_round))) {
    if (segs_all_round[i, 2] == segs_all_round[i, 4]) {
      mat[segs_all_round[i, 1]:segs_all_round[i, 3], segs_all_round[i, 2]] <-
        crayon::make_style(dcol, bg = FALSE)(char_across)
    } else if (segs_all_round[i, 1] == segs_all_round[i, 3]) {
      mat[segs_all_round[i, 1], segs_all_round[i, 2]:segs_all_round[i, 4]] <-
        crayon::make_style(dcol, bg = FALSE)(char_down)
    }
  }
  ...
  if (show_counts == TRUE) {
    if (nrow(mat) - 3 - max(rescaled_counts) == 0) {
      if (nchar(max(p$counts)) == 1) {
        mat[ 1, which(rescaled_counts == max(rescaled_counts)) ] <- 
          paste0(" ", max(p$counts), " ")
        ...
        mat <- rbind(
          matrix(c("       ", rep(crayon::reset("   "), ncol(mat) - 1)), nrow = 1), 
          mat)
        mat[1, which(rescaled_counts == max(rescaled_counts))] <- strsplit(
          formatC(p$counts[i], format = "e"), 
          split = "e")[1]
        mat[2, which(rescaled_counts == max(rescaled_counts))] <- strsplit(
          formatC(p$counts[i], format = "e"), 
          split = "e")[2]
      }
    }
